...
import { StaffMember, ShiftType, EmploymentStatus } from '../types';
...
interface AdminModalProps {
  ...
  currentDays?: number;
  onSaveSettings?: (url: string, days: number) => void;
...
export const AdminModal: React.FC<AdminModalProps> = ({ 
  ...
  // Settings State
  const [urlInput, setUrlInput] = useState(currentCsvUrl);
  ...
  }, [isOpen, currentCsvUrl, currentDays, staffData]);
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    e.preventDefault();
    if (onSaveSettings) {
      onSaveSettings(urlInput, daysInput);
  ...
  const handleDownloadTemplate = () => {
    ...
    const exampleRow = ['Example Staff', 'Manager', '2024-01-01', '5', '2', 'Normal', 'Permanent'];
    ...
    link.setAttribute("download", "protea_roster_template.csv");
  ...
  const renderEditForm = () => (
    <div className={`space-y-6 p-5 border transition-all duration-300 animate-fade-in ${
      isAddingNew 
           ...
           <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full animate-slide-in">
            <p className="font-sans text-[10px] tracking-widest uppercase mb-8 opacity-80 font-bold text-center text-fashion-black dark:text-fashion-white">Authorized Personnel Only</p>
             <form onSubmit={handleSubmitLogin} className="space-y-6">
              <div>
                <label className="block text-[10px] uppercase tracking-widest mb-2 text-center font-bold text-fashion-black dark:text-fashion-white">Passcode</label>
                <input 
                  type="password" 
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full bg-transparent border-b-2 border-fashion-black dark:border-fashion-white py-2 focus:outline-none focus:border-fashion-accent text-center font-serif text-xl text-fashion-black dark:text-fashion-white"
                  placeholder="••••"
                  autoFocus
                  maxLength={4}
                />
                {error && <p className="text-fashion-accent text-xs mt-2 text-center font-bold">Invalid Passcode</p>}
              </div>
              <button 
                type="submit" 
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
              ...
              {activeTab === 'settings' && (
                <form onSubmit={handleSaveSettings} className="space-y-6 animate-fade-in">
                  <div className="p-3 bg-fashion-gray/30 dark:bg-fashion-white/5 border border-fashion-black/20 dark:border-fashion-white/20 text-[10px] uppercase tracking-wide font-medium">
                    STATUS: {isUsingLocalData ? <span className="text-fashion-accent font-bold">USING LOCAL DATA</span> : <span className="text-green-700 dark:text-green-400 font-bold">LINKED TO CSV</span>}
                  </div>

                  <div>
                    <label className="block text-[10px] uppercase tracking-widest mb-2 opacity-80 font-bold">Roster Data Source (CSV)</label>
                    <div className="flex gap-2">
                      <input 
                        type="text" 
                        value={urlInput}
                        onChange={(e) => setUrlInput(e.target.value)}
                        className="flex-1 bg-transparent border-b border-fashion-black dark:border-fashion-white py-2 text-xs font-mono focus:outline-none focus:border-fashion-accent placeholder-fashion-black/30"
                        disabled={isUsingLocalData}
                        placeholder="https://..."
                      />
                      <button 
                        type="button"
                        onClick={handleTestLink}
                        className="text-[9px] uppercase tracking-widest border border-fashion-black/40 dark:border-fashion-white/40 px-3 hover:bg-fashion-black hover:text-fashion-white dark:hover:bg-fashion-white dark:hover:text-fashion-black transition-colors font-bold"
                  ...
                  <div className="pt-4 flex flex-col gap-4">
                    <button type="submit" className="w-full bg-fashion-black dark:bg-fashion-white text-fashion-white dark:text-fashion-black py-4 text-xs uppercase tracking-widest hover:opacity-90 font-bold">
                      Link CSV & Save Settings
                    </button>
                    
                    <button 
                      type="button" 
                      onClick={handleDownloadTemplate}
                      className="w-full py-3 border border-fashion-black dark:border-fashion-white text-[10px] uppercase tracking-widest hover:bg-fashion-gray/20 dark:hover:bg-fashion-white/10 transition-colors font-bold"
                    >
                  ...
                  {saveMessage && <div className="text-center text-[10px] uppercase tracking-widest text-fashion-accent font-bold">{saveMessage}</div>}
                </form>
              )}
                  ...
                  <div className="space-y-2">
                    {localStaff.map(person => {
                       ...
                       return (
                          <div 
                            key={person.id} 
                            className={`flex justify-between items-center p-3 border transition-all duration-300 ${
                              isBlurred 
